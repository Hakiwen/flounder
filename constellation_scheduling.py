...
def convert_constellation_scheduling(S, A_S, time, D,  BW, gamma, No, P_t, T, A_T, proc_time, out_size):
    assert S >= 1
    assert A_S.shape == (S, S)
    assert len(time) >= 1
    num_steps = len(time)
    time = np.array(time)
    t_step = (time[-1] - time[0])/num_steps
    # t_step = np.average(np.diff(time))
    ...
    for i in range(num_outs):
        for j in range(edge_list.shape[0]):
            capacity = BW*np.log(
                np.divide(1.0, np.power(D[edge_list[j][0], edge_list[j][1], :], gamma))*(P_t/No)
            )
